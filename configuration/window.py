...
class Window(QMainWindow):
    """Main window for selecting a project type."""
    def __init__(self):
        """Initialize the Window and load the UI."""
        super().__init__()
        ui_file=resource_path("Gui/select_gui.ui")
...
###########  defining the new and existing project window   #####################
    ...
    def open_existing_project_dialog(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Open Config File", "", "Text Files (*.txt);;All Files (*)", options=options
        )
        if file_path:
            ...
            if os.path.basename(normalized_file_path) == "config.txt":
                try:
                    ...
                    with open(normalized_file_path, 'r') as file:
                        ...
                        self.newwindow.db_file_path = os.path.join(
                            project_folder,project_name,"data_logger.db"
                            ).replace('\\', '/')
