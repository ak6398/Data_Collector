...
from .data_thread import DataFetchThread
...
class Form(QMainWindow):
    def __init__(self, ip_address, port, db_file_path):
        ...
        # Menubar for adding new IP address
        ip_menubar = self.menuBar()
        ip_menu = ip_menubar.findChild(QMenu, 'menuFile')
        new_file = QAction("Add IP Network", self)
        new_file.setShortcut("ctrl+I")
        ip_menu.addAction(new_file)
        ...
        self.data_fetch_thread = DataFetchThread(ip_address, port)
    ...
    def export_manage(self):
        from configuration.test_progress_prac import MyWidget
